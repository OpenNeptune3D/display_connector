    ...
    def __init__(self) -> None:
        ...
        self.data_mapping = {
            ...
            "print_stats": {
                "print_duration": [
                    MappingLeaf(
                        [build_accessor(self.map_page(PAGE_PRINTING), "6"), build_accessor(self.map_page(PAGE_PRINTING_COMPLETE), "4")],
                        formatter=format_time,
                    )
                ],
                "filename": [
                    MappingLeaf(
                        [build_accessor(self.map_page(PAGE_PRINTING), "t0"), build_accessor(self.map_page(PAGE_PRINTING_COMPLETE), "3")],
...
class ElegooDisplayCommunicator(DisplayCommunicator):
    ...
    async def special_page_handling(self, current_page):
        if current_page == PAGE_MAIN:
            ...
            if self.display_name_override:
                ...
                await self.write('xstr 12,20,180,20,1,65535,' + str(BACKGROUND_GRAY) + ',0,1,1,"' + display_name + '"')
            ...
            await self.write("p[" + self.mapper.map_page(PAGE_SETTINGS_ABOUT) +"].b[16].txt=\"" + self.ips + "\"")
            await self.write('fill 0,400,320,60,' + str(BACKGROUND_GRAY))
            await self.write('xstr 0,400,320,30,1,65535,' + str(BACKGROUND_GRAY) + ',1,1,1,"OpenNept4une"')
            await self.write('xstr 0,430,320,30,2,GRAY,' + str(BACKGROUND_GRAY) + ',1,1,1,"github.com/OpenNeptune3D"')
    ...
    async def update_printing_heater_settings_ui(self, printing_selected_heater, printing_target_temp):
        if self.has_two_beds:
            ...
            await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_FILAMENT)}].b1.picc=' + str(90 if printing_selected_heater == "heater_bed" else 89))
            await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_FILAMENT)}].b2.picc=' + str(90 if printing_selected_heater == "heater_bed_outer" else 89))
            await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_FILAMENT)}].targettemp.val=' + str(printing_target_temp))

        else:
            await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_FILAMENT)}].b[13].pic={54 + ["extruder", "heater_bed"].index(printing_selected_heater)}')
            await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_FILAMENT)}].b[35].txt="' + str(printing_target_temp) + '"')


    async def update_printing_temperature_increment_ui(self, printing_selected_temp_increment):
        await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_FILAMENT)}].p1.pic={56 + ["1", "5", "10"].index(printing_selected_temp_increment)}')

    async def update_printing_speed_settings_ui(self, printing_selected_speed_type, printing_target_speed):
        await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_SPEED)}].b0.picc=' + str(59 if printing_selected_speed_type == "print" else 58))
        await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_SPEED)}].b1.picc=' + str(59 if printing_selected_speed_type == "flow" else 58))
        await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_SPEED)}].b2.picc=' + str(59 if printing_selected_speed_type == "fan" else 58))
        await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_SPEED)}].targetspeed.val={printing_target_speed*100:.0f}')

    async def update_printing_speed_increment_ui(self, printing_selected_speed_increment):
        await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_SPEED)}].b[14].picc=' + str(59 if printing_selected_speed_increment == "1" else 58))
        await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_SPEED)}].b[15].picc=' + str(59 if printing_selected_speed_increment == "5" else 58))
        await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_SPEED)}].b[16].picc=' + str(59 if printing_selected_speed_increment == "10" else 58))

    async def update_printing_zoffset_increment_ui(self, z_offset_distance):
        await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_ADJUST)}].b[23].picc=' + str(36 if z_offset_distance == "0.01" else 65))
        await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_ADJUST)}].b[24].picc=' + str(36 if z_offset_distance == "0.1" else 65))
        await self.write(f'p[{self.mapper.map_page(PAGE_PRINTING_ADJUST)}].b[25].picc=' + str(36 if z_offset_distance == "1" else 65))

    async def update_preset_temp_ui(self, temperature_preset_step, temperature_preset_extruder, temperature_preset_bed):
        await self.write(f'p[{self.mapper.map_page(PAGE_SETTINGS_TEMPERATURE_SET)}].b[7].pic={56 + [1, 5, 10].index(temperature_preset_step)}')
        await self.write(f'p[{self.mapper.map_page(PAGE_SETTINGS_TEMPERATURE_SET)}].b[18].txt="{temperature_preset_extruder}"')
        await self.write(f'p[{self.mapper.map_page(PAGE_SETTINGS_TEMPERATURE_SET)}].b[19].txt="{temperature_preset_bed}"')

    async def update_prepare_move_ui(self, move_distance):
        await self.write(f'p[{self.mapper.map_page(PAGE_PREPARE_MOVE)}].p0.pic={10 + ["0.1", "1", "10"].index(move_distance)}')
    ...
    async def update_wifi_ui(self):
        ...
        if not has_wifi:
            await self.write(f'picq 230,0,42,42,214')
            return False
        if ssid is None:
            await self.write(f'picq 230,0,42,42,313')
        else:
    ...
    async def show_files_page(self, current_dir, dir_contents, files_page):
        ...
        if file_count == 0:
                await self.write(f'p[{self.mapper.map_page(PAGE_FILES)}].b[11].txt="{title} (Empty)"')
        else:
            await self.write(f'p[{self.mapper.map_page(PAGE_FILES)}].b[11].txt="{title} ({(files_page * page_size) + 1}-{min((files_page * page_size) + page_size, file_count)}/{file_count})"')
        component_index = 0
        for index in range(files_page * page_size, min(len(dir_contents), (files_page + 1) * page_size)):
            file = dir_contents[index]
            await self.write(f'p[{self.mapper.map_page(PAGE_FILES)}].b[{component_index + 18}].txt="{file["name"]}"')
            if file["type"] == "dir":
                await self.write(f'p[{self.mapper.map_page(PAGE_FILES)}].b[{component_index + 13}].pic=194')
            else:
                await self.write(f'p[{self.mapper.map_page(PAGE_FILES)}].b[{component_index + 13}].pic=193')
            component_index += 1
        for index in range(component_index, page_size):
            await self.write(f'p[{self.mapper.map_page(PAGE_FILES)}].b[{index + 13}].pic=195')
            await self.write(f'p[{self.mapper.map_page(PAGE_FILES)}].b[{index + 18}].txt=""')
    ...
    async def set_data_prepare_screen(self, filename, metadata):
        ...
        await self.write(f'p[{self.mapper.map_page(PAGE_CONFIRM_PRINT)}].b[2].txt="{build_format_filename()(filename)}"')
        info = []
        ...
        await self.write(f'p[{self.mapper.map_page(PAGE_CONFIRM_PRINT)}].b[3].txt="{" ".join(info)}"')
    ...
    async def draw_completed_screw_leveling(self, screw_levels):
        ...
        await self.write("b[3].txt=\"01:20 means 1  turn and 20 mins\\rCW=clockwise\\rCCW=counter-clockwise\"")
        await self.write("vis b[4],0")
        await self.write("vis b[5],0")
        await self.write("vis b[6],0")
        await self.write("vis b[7],0")
    ...
    async def update_zprobe_leveling_ui(self, z_probe_step, z_probe_distance):
        await self.write("p[137].b[19].txt=\"Z-Probe\"")
        await self.write(f'p[137].b[11].pic={7 + ["0.01", "0.1", "1"].index(z_probe_step)}')
    ...
    async def draw_kamp_box(self, x, y, color):
        ...
        if box_size > 0:
            await self.write(f'fill {int(self.bed_leveling_x_offset+x*(box_size+2))},{47+y*(box_size+2)},{box_size},{box_size},{color}')

    async def update_klipper_version_ui(self, software_version):
        await self.write("p[" + self.mapper.map_page(PAGE_SETTINGS_ABOUT) + "].b[10].txt=\"" + software_version + "\"")
