...
class ConfigHandler(ConfigParser):
    ...
    def initialize_config_file(self):
        if not os.path.exists(self.file_path):
            ...
            self.set('general', 'clean_filename_regex', '.*_(.*?_(?:[0-9]+h|[0-9]+m|[0-9]+s)+\.gcode)')
            ...
            self.set('main_screen', '; set to MODEL_NAME for built in model name. Remove to use Elegoo model images.')
            self.set('main_screen', 'display_name', 'MODEL_NAME')
            self.set('main_screen', '; color for the line below the model name. As RGB565 value.')
            ...
            self.set('main_screen', '; Background color for thumbnails. As RGB Hex value. Remove for default background color.')
